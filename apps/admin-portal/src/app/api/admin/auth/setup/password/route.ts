...
import { AdminAlreadyExistsError, PasswordPolicyError, createStaffService } from '@deskatlas/domain';
...
export async function POST(request: NextRequest) {
  try {
    ...
    let { userId, email, displayName, password, token, code } = body;
    ...
    if (code && supabaseUrl && anonKey && !token) {
      try {
        ...
        console.warn('[Admin Setup Password] Code exchange fallback error:', exchangeErr);
      }
    }

    // If an OAuth access token is provided, verify it directly with Supabase Auth
    if (token && supabaseUrl && anonKey) {
      try {
        ...
        console.warn('[Admin Setup Password] Token verification fallback error:', authErr);
      }
    }

    if (!userId || typeof userId !== 'string') {
      return NextResponse.json(
        { error: 'Valid user ID is required to set administrator password.' },
        { status: 400 }
      );
    }
